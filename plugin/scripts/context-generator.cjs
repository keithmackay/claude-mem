...
`+JSON.stringify(i,null,2):T=" "+this.formatData(i));let b="";if(n){let{sessionId:f,sdkSessionId:D,correlationId:l,...r}=n;Object.keys(r).length>0&&(b=` {${Object.entries(r).map(([O,g])=>`${O}=${g}`).join(", ")}}`)}let v=`[${a}] [${c}] [${p}] ${h}${t}${b}${T}`;e===3?console.error(v):console.log(v)}debug(e,s,t,n){this.log(0,e,s,t,n)}info(e,s,t,n){this.log(1,e,s,t,n)}warn(e,s,t,n){this.log(2,e,s,t,n)}error(e,s,t,n){this.log(3,e,s,t,n)}dataIn(e,s,t,n){this.info(e,`\u2192 ${s}`,t,n)}dataOut(e,s,t,n){this.info(e,`\u2190 ${s}`,t,n)}success(e,s,t,n){this.info(e,`\u2713 ${s}`,t,n)}failure(e,s,t,n){this.error(e,`\u2717 ${s}`,t,n)}timing(e,s,t,n){this.info(e,`\u23F1 ${s}`,n,{duration:`${t}ms`})}},U=new oe;var x=class{static DEFAULTS={CLAUDE_MEM_MODEL:"claude-haiku-4-5",CLAUDE_MEM_CONTEXT_OBSERVATIONS:"50",CLAUDE_MEM_WORKER_PORT:"37777",CLAUDE_MEM_SKIP_TOOLS:"ListMcpResourcesTool,SlashCommand,Skill,TodoWrite,AskUserQuestion",CLAUDE_MEM_DATA_DIR:(0,Se.join)((0,he.homedir)(),".claude-mem"),CLAUDE_MEM_LOG_LEVEL:"INFO",CLAUDE_MEM_PYTHON_VERSION:"3.13",CLAUDE_CODE_PATH:"",CLAUDE_MEM_EMBEDDING_PROVIDER:"ollama",CLAUDE_MEM_OLLAMA_EMBED_MODEL:"all-minilm",CLAUDE_MEM_OLLAMA_BASE_URL:"http://localhost:11434",CLAUDE_MEM_CONTEXT_SHOW_READ_TOKENS:"true",CLAUDE_MEM_CONTEXT_SHOW_WORK_TOKENS:"true",CLAUDE_MEM_CONTEXT_SHOW_SAVINGS_AMOUNT:"true",CLAUDE_MEM_CONTEXT_SHOW_SAVINGS_PERCENT:"true",CLAUDE_MEM_CONTEXT_OBSERVATION_TYPES:Te,CLAUDE_MEM_CONTEXT_OBSERVATION_CONCEPTS:ge,CLAUDE_MEM_CONTEXT_FULL_COUNT:"5",CLAUDE_MEM_CONTEXT_FULL_FIELD:"narrative",CLAUDE_MEM_CONTEXT_SESSION_COUNT:"10",CLAUDE_MEM_CONTEXT_SHOW_LAST_SUMMARY:"true",CLAUDE_MEM_CONTEXT_SHOW_LAST_MESSAGE:"false"};static getAllDefaults(){return{...this.DEFAULTS}}static get(e){return this.DEFAULTS[e]}static getInt(e){let s=this.get(e);return parseInt(s,10)}static getBool(e){return this.get(e)==="true"}static loadFromFile(e){if(!(0,W.existsSync)(e))return this.getAllDefaults();let s=(0,W.readFileSync)(e,"utf-8"),t=JSON.parse(s),n=t;if(t.env&&typeof t.env=="object"){n=t.env;try{(0,W.writeFileSync)(e,JSON.stringify(n,null,2),"utf-8"),U.info("SETTINGS","Migrated settings file from nested to flat schema",{settingsPath:e})}catch(a){U.warn("SETTINGS","Failed to auto-migrate settings file",{settingsPath:e},a)}}let i={...this.DEFAULTS};for(let a of Object.keys(this.DEFAULTS))n[a]!==void 0&&(i[a]=n[a]);return i}};var Be={};function Pe(){return typeof __dirname<"u"?__dirname:(0,S.dirname)((0,Oe.fileURLToPath)(Be.url))}var ps=Pe(),C=x.get("CLAUDE_MEM_DATA_DIR"),ie=process.env.CLAUDE_CONFIG_DIR||(0,S.join)((0,be.homedir)(),".claude"),us=(0,S.join)(C,"archives"),ls=(0,S.join)(C,"logs"),Es=(0,S.join)(C,"trash"),ms=(0,S.join)(C,"backups"),Ts=(0,S.join)(C,"settings.json"),Re=(0,S.join)(C,"claude-mem.db"),gs=(0,S.join)(C,"vector-db"),Ss=(0,S.join)(ie,"settings.json"),hs=(0,S.join)(ie,"commands"),bs=(0,S.join)(ie,"CLAUDE.md");function Ne(d){(0,fe.mkdirSync)(d,{recursive:!0})}var J=class{db;constructor(){Ne(C),this.db=new Ae.default(Re),this.db.pragma("journal_mode = WAL"),this.db.pragma("synchronous = NORMAL"),this.db.pragma("foreign_keys = ON"),this.initializeSchema(),this.ensureWorkerPortColumn(),this.ensurePromptTrackingColumns(),this.removeSessionSummariesUniqueConstraint(),this.addObservationHierarchicalFields(),this.makeObservationsTextNullable(),this.createUserPromptsTable(),this.ensureDiscoveryTokensColumn()}initializeSchema(){try{this.db.exec(`
    ...
    `).get(e)?.worker_port||null}saveUserPrompt(e,s,t){let n=new Date,i=n.getTime();return this.db.prepare(`
      ...
      VALUES (?, ?, ?, ?, ?)
    `).run(e,s,t,n.toISOString(),i).lastInsertRowid}saveAssistantResponse(e,s,t){let n=new Date,i=n.getTime();return this.db.prepare(`
    ...
    `).all(...e)}getUserPromptsByIds(e,s={}){if(e.length===0)return[];let{orderBy:t="date_desc",limit:n}=s,i=t==="date_asc"?"ASC":"DESC",a=n?`LIMIT ${n}`:"",c=e.map(()=>"?").join(",");return this.db.prepare(`
      ...
      ${a}
    `).all(...e)}getTimelineAroundTimestamp(e,s=10,t=10,n){return this.getTimelineAroundObservation(null,e,s,t,n)}getTimelineAroundObservation(e,s,t=10,n=10,i){let a=i?"AND project = ?":"",c=i?[i]:[],p,h;if(e!==null){let f=`
...
`).filter(n=>n.trim()),t="";for(let n=s.length-1;n>=0;n--)try{let i=s[n];if(!i.includes('"type":"assistant"'))continue;let a=JSON.parse(i);if(a.type==="assistant"&&a.message?.content&&Array.isArray(a.message.content)){let c="";for(let p of a.message.content)p.type==="text"&&(c+=p.text);if(c=c.replace(/<system-reminder>[\s\S]*?<\/system-reminder>/g,"").trim(),c){t=c;break}}}catch{continue}return{userMessage:"",assistantMessage:t}}catch(e){return U.failure("WORKER","Failed to extract prior messages from transcript",{transcriptPath:d},e),{userMessage:"",assistantMessage:""}}}async function Qe(d,e=!1){let s=He(),t=d?.cwd??process.cwd(),n=t?w.default.basename(t):"unknown-project",i=null;try{i=new J}catch(L){if(L.code==="ERR_DLOPEN_FAILED"){try{(0,H.unlinkSync)(We)}catch{}return console.error("Native module rebuild needed - restart Claude Code to auto-fix"),""}throw L}let a=Array.from(s.observationTypes),c=a.map(()=>"?").join(","),p=Array.from(s.observationConcepts),h=p.map(()=>"?").join(","),T=i.db.prepare(`
    ...
    ORDER BY created_at_epoch DESC
    LIMIT ?
  `).all(n,...a,...p,s.totalObservationCount),b=i.db.prepare(`
    SELECT id, sdk_session_id, request, investigated, learned, completed, next_steps, created_at, created_at_epoch
    FROM session_summaries
    WHERE project = ?
    ORDER BY created_at_epoch DESC
    LIMIT ?
  `).all(n,s.sessionCount+je),v="",f="";if(s.showLastMessage&&T.length>0)try{let L=d?.session_id,O=T.find(g=>g.sdk_session_id!==L);if(O){let g=O.sdk_session_id,y=qe(t),F=w.default.join((0,z.homedir)(),".claude","projects",y,`${g}.jsonl`),j=Je(F);v=j.userMessage,f=j.assistantMessage}}catch{}if(T.length===0&&b.length===0)return i?.close(),e?`
...
No previous sessions found for this project yet.`;let D=b.slice(0,s.sessionCount),l=T,r=[];if(e?(r.push(""),r.push(`${o.bright}${o.cyan}[${n}] recent context${o.reset}`),r.push(`${o.gray}${"\u2500".repeat(60)}${o.reset}`),r.push("")):(r.push(`# [${n}] recent context`),r.push("")),l.length>0){e?r.push(`${o.dim}Legend: \u{1F3AF} session-request | \u{1F534} bugfix | \u{1F7E3} feature | \u{1F504} refactor | \u2705 change | \u{1F535} discovery | \u2696\uFE0F  decision${o.reset}`):r.push("**Legend:** \u{1F3AF} session-request | \u{1F534} bugfix | \u{1F7E3} feature | \u{1F504} refactor | \u2705 change | \u{1F535} discovery | \u2696\uFE0F  decision"),r.push(""),e?(r.push(`${o.bright}\u{1F4A1} Column Key${o.reset}`),r.push(`${o.dim}  Read: Tokens to read this observation (cost to learn it now)${o.reset}`),r.push(`${o.dim}  Work: Tokens spent on work that produced this record (\u{1F50D} research, \u{1F6E0}\uFE0F building, \u2696\uFE0F  deciding)${o.reset}`)):(r.push("\u{1F4A1} **Column Key**:"),r.push("- **Read**: Tokens to read this observation (cost to learn it now)"),r.push("- **Work**: Tokens spent on work that produced this record (\u{1F50D} research, \u{1F6E0}\uFE0F building, \u2696\uFE0F  deciding)")),r.push(""),e?(r.push(`${o.dim}\u{1F4A1} Context Index: This semantic index (titles, types, files, tokens) is usually sufficient to understand past work.${o.reset}`),r.push(""),r.push(`${o.dim}When you need implementation details, rationale, or debugging context:${o.reset}`),r.push(`${o.dim}  - Use the mem-search skill to fetch full observations on-demand${o.reset}`),r.push(`${o.dim}  - Critical types (\u{1F534} bugfix, \u2696\uFE0F decision) often need detailed fetching${o.reset}`),r.push(`${o.dim}  - Trust this index over re-reading code for past decisions and learnings${o.reset}`)):(r.push("\u{1F4A1} **Context Index:** This semantic index (titles, types, files, tokens) is usually sufficient to understand past work."),r.push(""),r.push("When you need implementation details, rationale, or debugging context:"),r.push("- Use the mem-search skill to fetch full observations on-demand"),r.push("- Critical types (\u{1F534} bugfix, \u2696\uFE0F decision) often need detailed fetching"),r.push("- Trust this index over re-reading code for past decisions and learnings")),r.push("");let L=T.length,O=T.reduce((_,E)=>{let m=(E.title?.length||0)+(E.subtitle?.length||0)+(E.narrative?.length||0)+JSON.stringify(E.facts||[]).length;return _+Math.ceil(m/Le)},0),g=T.reduce((_,E)=>_+(E.discovery_tokens||0),0),y=g-O,F=g>0?Math.round(y/g*100):0,j=s.showReadTokens||s.showWorkTokens||s.showSavingsAmount||s.showSavingsPercent;if(j)if(e){if(r.push(`${o.bright}${o.cyan}\u{1F4CA} Context Economics${o.reset}`),r.push(`${o.dim}  Loading: ${L} observations (${O.toLocaleString()} tokens to read)${o.reset}`),r.push(`${o.dim}  Work investment: ${g.toLocaleString()} tokens spent on research, building, and decisions${o.reset}`),g>0&&(s.showSavingsAmount||s.showSavingsPercent)){let _="  Your savings: ";s.showSavingsAmount&&s.showSavingsPercent?_+=`${y.toLocaleString()} tokens (${F}% reduction from reuse)`:s.showSavingsAmount?_+=`${y.toLocaleString()} tokens`:_+=`${F}% reduction from reuse`,r.push(`${o.green}${_}${o.reset}`)}r.push("")}else{if(r.push("\u{1F4CA} **Context Economics**:"),r.push(`- Loading: ${L} observations (${O.toLocaleString()} tokens to read)`),r.push(`- Work investment: ${g.toLocaleString()} tokens spent on research, building, and decisions`),g>0&&(s.showSavingsAmount||s.showSavingsPercent)){let _="- Your savings: ";s.showSavingsAmount&&s.showSavingsPercent?_+=`${y.toLocaleString()} tokens (${F}% reduction from reuse)`:s.showSavingsAmount?_+=`${y.toLocaleString()} tokens`:_+=`${F}% reduction from reuse`,r.push(_)}r.push("")}let Ce=b[0]?.id,ve=D.map((_,E)=>{let m=E===0?null:b[E+1];return{..._,displayEpoch:m?m.created_at_epoch:_.created_at_epoch,displayTime:m?m.created_at:_.created_at,shouldShowLink:_.id!==Ce}}),De=new Set(T.slice(0,s.fullObservationCount).map(_=>_.id)),ae=[...l.map(_=>({type:"observation",data:_})),...ve.map(_=>({type:"summary",data:_}))];ae.sort((_,E)=>{let m=_.type==="observation"?_.data.created_at_epoch:_.data.displayEpoch,M=E.type==="observation"?E.data.created_at_epoch:E.data.displayEpoch;return m-M});let G=new Map;for(let _ of ae){let E=_.type==="observation"?_.data.created_at:_.data.displayTime,m=Ye(E);G.has(m)||G.set(m,[]),G.get(m).push(_)}let ye=Array.from(G.entries()).sort((_,E)=>{let m=new Date(_[0]).getTime(),M=new Date(E[0]).getTime();return m-M});for(let[_,E]of ye){e?(r.push(`${o.bright}${o.cyan}${_}${o.reset}`),r.push("")):(r.push(`### ${_}`),r.push(""));let m=null,M="",$=!1;for(let Z of E)if(Z.type==="summary"){$&&(r.push(""),$=!1,m=null,M="");let u=Z.data,X=`${u.request||"Session started"} (${Ge(u.displayTime)})`,I=u.shouldShowLink?`claude-mem://session-summary/${u.id}`:"";if(e){let R=I?`${o.dim}[${I}]${o.reset}`:"";r.push(`\u{1F3AF} ${o.yellow}#S${u.id}${o.reset} ${X} ${R}`)}else{let R=I?` [\u2192](${I})`:"";r.push(`**\u{1F3AF} #S${u.id}** ${X}${R}`)}r.push("")}else{let u=Z.data,X=Ie(u.files_modified),I=X.length>0&&X[0]?Ke(X[0],t):"General";I!==m&&($&&r.push(""),e?r.push(`${o.dim}${I}${o.reset}`):r.push(`**${I}**`),e||(r.push("| ID | Time | T | Title | Read | Work |"),r.push("|----|------|---|-------|------|------|")),m=I,$=!0,M="");let R=Ve(u.created_at),V=u.title||"Untitled",Y=Ee[u.type]||"\u2022",Me=(u.title?.length||0)+(u.subtitle?.length||0)+(u.narrative?.length||0)+JSON.stringify(u.facts||[]).length,P=Math.ceil(Me/Le),B=u.discovery_tokens||0,ee=me[u.type]||"\u{1F50D}",ce=B>0?`${ee} ${B.toLocaleString()}`:"-",se=R!==M,_e=se?R:"";if(M=R,De.has(u.id)){let k=s.fullObservationField==="narrative"?u.narrative:u.facts?Ie(u.facts).join(`
`):null;if(e){let A=se?`${o.dim}${R}${o.reset}`:" ".repeat(R.length),K=s.showReadTokens&&P>0?`${o.dim}(~${P}t)${o.reset}`:"",pe=s.showWorkTokens&&B>0?`${o.dim}(${ee} ${B.toLocaleString()}t)${o.reset}`:"";r.push(`  ${o.dim}#${u.id}${o.reset}  ${A}  ${Y}  ${o.bright}${V}${o.reset}`),k&&r.push(`    ${o.dim}${k}${o.reset}`),(K||pe)&&r.push(`    ${K} ${pe}`),r.push("")}else{$&&(r.push(""),$=!1),r.push(`**#${u.id}** ${_e||"\u2033"} ${Y} **${V}**`),k&&(r.push(""),r.push(k),r.push(""));let A=[];s.showReadTokens&&A.push(`Read: ~${P}`),s.showWorkTokens&&A.push(`Work: ${ce}`),A.length>0&&r.push(A.join(", ")),r.push(""),m=null}}else if(e){let k=se?`${o.dim}${R}${o.reset}`:" ".repeat(R.length),A=s.showReadTokens&&P>0?`${o.dim}(~${P}t)${o.reset}`:"",K=s.showWorkTokens&&B>0?`${o.dim}(${ee} ${B.toLocaleString()}t)${o.reset}`:"";r.push(`  ${o.dim}#${u.id}${o.reset}  ${k}  ${Y}  ${V} ${A} ${K}`)}else{let k=s.showReadTokens?`~${P}`:"",A=s.showWorkTokens?ce:"";r.push(`| #${u.id} | ${_e||"\u2033"} | ${Y} | ${V} | ${k} | ${A} |`)}}$&&r.push("")}let N=b[0],de=T[0];if(s.showLastSummary&&N&&(N.investigated||N.learned||N.completed||N.next_steps)&&(!de||N.created_at_epoch>de.created_at_epoch)&&(r.push(...Q("Investigated",N.investigated,o.blue,e)),r.push(...Q("Learned",N.learned,o.yellow,e)),r.push(...Q("Completed",N.completed,o.green,e)),r.push(...Q("Next Steps",N.next_steps,o.magenta,e))),f&&(r.push(""),r.push("---"),r.push(""),e?(r.push(`${o.bright}${o.magenta}\u{1F4CB} Previously${o.reset}`),r.push(""),r.push(`${o.dim}A: ${f}${o.reset}`)):(r.push("**\u{1F4CB} Previously**"),r.push(""),r.push(`A: ${f}`)),r.push("")),j&&g>0&&y>0){let _=Math.round(g/1e3);r.push(""),e?r.push(`${o.dim}\u{1F4B0} Access ${_}k tokens of past research & decisions for just ${O.toLocaleString()}t. Use the mem-search skill to access memories by ID instead of re-reading files.${o.reset}`):r.push(`\u{1F4B0} Access ${_}k tokens of past research & decisions for just ${O.toLocaleString()}t. Use the mem-search skill to access memories by ID instead of re-reading files.`)}}return i?.close(),r.join(`
